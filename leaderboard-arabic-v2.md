...
Finally, we also introduce the ALRAGE task: Arabic Language Retrieval Augmented Generation Evaluation. It's based on the new metric from lighteval: LLM-as-a-judge
The evaluation process begins with inputs from the ALRAGE dataset [https://huggingface.co/datasets/OALL/ALRAGE](https://huggingface.co/datasets/OALL/ALRAGE-Benchmark#alrage-arabic-language-rag-evaluation-benchmark) , which includes a question, a set of candidate contexts, and a gold answer, along with the generated answer from the model being evaluated. The core of the evaluation relies on Qwen2.5-72B-Instruct as a judge model, which receives a carefully structured Arabic prompt containing the original question, the model's generated answer, and the gold answer.
